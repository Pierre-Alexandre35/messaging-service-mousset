...
class Pagination():
    ...
    def __init__(self, total_items, items_per_page, page_range):
        self.total_items = total_items  # total number of article to display
        self.items_per_page = items_per_page  # max nbr of article/page
        self.page_range = page_range  # nbr of prev and next pages to display
        self.current_page = 0
        self.number_of_pages = math.ceil(
            self.total_items / self.items_per_page)
        # if the current_page is 7 and page_range is 3
        # then self.following will be [8, 9, 10] and self.previous [4, 5 ,6]
        self.following = list(
            range(self.current_page + 1,
                  self.current_page + self.page_range + 1))
    ...
    def number_of_previous_items(self):
        """
        Sum of all items in all previous pages
